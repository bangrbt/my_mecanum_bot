...
Package `my_mecanum_bot` cung cấp môi trường mô phỏng toàn diện cho robot di chuyển đa hướng (Mecanum) trên nền tảng **ROS 2 Humble** và **Gazebo Classic**. Hệ thống tích hợp điều khiển động học (Kinematics), tay máy (Manipulator) 2 bậc tự do, cấu hình cảm biến (LiDAR, Camera), và tính năng tự động sinh quỹ đạo trực quan trên RViz2.
...
- **Hệ thống Cảm biến (Sensors):** Mô phỏng LiDAR quét 360 độ và Camera RGB với luồng dữ liệu thời gian thực được map qua ROS topics.
